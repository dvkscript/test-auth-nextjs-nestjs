...
import { AggregateRoot, Mapper, Paginated, PaginatedQueryParams, RepositoryPort } from '../ddd';
...
> implements RepositoryPort<Aggregate> {
  ...
  async insert(entity: Aggregate | Aggregate[]): Promise<void> {
    ...
    try {
      ...
      if (error instanceof UniqueIntegrityConstraintViolationError) {
        this.logger.debug(
          `[${RequestContextService.getRequestId()}] ${(error.cause as any).detail
          }`,
        );
  ...
  protected generateInsertQuery(
    ...
    return this.simpleGenerateInsertQuery(this.tableName, models);
  }

  protected simpleGenerateInsertQuery<T extends Record<string, any>>(tableName: string, models: T[]): QuerySqlToken {
